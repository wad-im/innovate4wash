...
const SessionCard = ({ sessionDetails }) => {
  ...
  const presentations =
    Presentations !== null &&
    Presentations.map(presentation => presentation.data)
  console.log(presentations)

  return (
    <SessionCardContainer sessionType={Type[0]} className="session-card">
      <p>
        {Start.substr(3)} - {End.substr(3)}
      </p>
      <div className="session-main">
        ...
        {presentations && (
          <button className="expand-session" onClick={expandInfo}>
            ...
            presentations.map(pitch => (
              ...
              >
                <div className="pitch-main">
                  ...
                  {pitch.Attachments && (
                    <a
                      className="pitch-slides"
                      href={pitch.Attachments[0].url}
                      target="_blank"
                      rel="noreferrer noopener"
                    >
                      Get the pitch slides
                    </a>
                  )}
                </div>
                    ...
                    pitch.Speaker.map(eachSpeaker => (
                      <div key={eachSpeaker.data.Record_Id} className="speaker">
                        <p className="speaker-name">
                          by {eachSpeaker.data.Name && eachSpeaker.data.Name}
                        </p>
                        <p>
                          {eachSpeaker.data.Website ? (
                            <a href={eachSpeaker.data.Website}>
                              {eachSpeaker.data.Organization}
                            </a>
                          ) : (
                            eachSpeaker.data.Organization
                          )}
...
const SessionCardContainer = styled.li`
  ...
  /* color: #9E8C2C; */
  color: ${({ sessionType }) => sessionType === "Sponsor Session" && "#EBCD36"};
  .session-main {
    display: grid;
  ...
  .expand-session {
    ...
    border: none;
    color: ${({ sessionType }) =>
      sessionType === "Sponsor Session" ? "#EBCD36" : "#4DEBEB"};
