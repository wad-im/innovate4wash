...
const ParticipatingOrganization = () => {
  ...
  data.allAirtable.edges.forEach(element => {
    ...
    let isIncludedInList = participatingOrganizations.some(
      organization => organization["Organization"] === Organization
    )
    if (Complete_Registration && !isIncludedInList) {
        ...
        {participatingOrganizations.map(organization => (
          <span key={organization.Record_Id} className="tag">
            {organization.Website ? (
              <a href={organization.Website}>{organization.Organization}</a>
            ) : (
...
const Container = styled.div`
  ...
  .tag {
    ...
    outline-offset: 2px;
    box-shadow: 0 0.0625rem 0.1875rem 0rem rgba(0, 0, 0, 0.1),
      0 0.0625rem 0.125rem -0.125rem rgba(0, 0, 0, 0.06);
    min-height: 2rem;
    min-width: 2rem;
